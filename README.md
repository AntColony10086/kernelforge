...
Given a PyTorch reference operator (20 supported in the MVP, covering normalization / activation / reduction / elementwise / geometric / linalg families — see Benchmark Suite below), KernelForge generates an MLX/Metal kernel, runs it against a **hidden holdout suite** (4–5 cases per op covering shape, dtype, layout, and selected numerical edge conditions), iterates with structured diff feedback until verified or capped, and — *if a kernel ever clears the holdouts* — benchmarks it against MLX eager / `mx.compile` / `mx.fast` built-ins. (The live run reported in [Scorecard](#scorecard-live-20-op-run) verified zero kernels and therefore produced no perf numbers; the harness is implemented in `kernel_lab/bench_tool.py` and wired into the agent.)
...
- **Cost-aware LLM routing.** Cheap `deepseek-v4-flash` on the happy path; escalate to `deepseek-coder` only after a real failure. (`deepseek-v4-pro` was the original escalation candidate but its long-prompt streaming was unreliable for kernel generation; `deepseek-coder` is the actually-used escalation route.)
...
Each op has 4–5 holdout cases that vary shape, dtype, layout, and selected numerical edge conditions. The kernel only graduates to `verified_correct` when EVERY holdout passes within tolerance `(max_abs_diff <= 1e-4, max_rel_diff <= 1e-3)`. Example RoPE holdouts:
...
- `shape=[2,24,128]` float32 — non-power-of-two sequence handling.
...
**Read**: Naive (smoke-only) declared 17 of 20 kernels correct because they compiled. KernelForge verified 0 of them against the hidden holdout suite — every single LLM-generated kernel had at least one holdout failure. **The contract held: zero false-success claims** while naive shipped 17 unverified kernels.

> *KernelForge did not produce a verified kernel in this run. That is the contract working as designed: when DeepSeek's first-attempt kernels did not survive the holdout suite, KernelForge refused to ship them. The result is **not "0 kernels optimized"; it is "0 false correctness claims"** — refusing to ship unverifiable GPU code is a production behavior, not a failure mode.*

The 2-min demo video uses a tighter 3-op scenario (with deterministic chaos on RoPE) where the false-success contrast is sharper for a video viewer; the 20-op run above is the breadth evidence.
...
## What we do NOT claim (preempting "is this fake?" questions)

In the interest of being fair to judges and graders, here is exactly what KernelForge is *not*:

- **We do not claim our LLM-generated kernels are fast.** The bench harness exists in `kernel_lab/bench_tool.py`, but the live 20-op run verified zero kernels and therefore measured zero speedups. KernelForge is a *verification gate*, not a performance optimiser. If a kernel passes the holdouts, it gets benched honestly against MLX built-ins. None did in this run.
- **We do not claim full TrueFoundry SaaS integration.** The agent talks to a local `local_gateway` FastAPI proxy at `127.0.0.1:8765` that mimics TrueFoundry AI Gateway routing semantics (cost-aware escalation via `X-TFY-METADATA: escalate=pro` → `deepseek-coder`). The proxy adds an `x-local-gateway: yes` header on every response so the demo never claims to be TrueFoundry itself. Drop in real `TFY_GATEWAY_BASE_URL` + `TFY_GATEWAY_API_KEY` env vars and the same client code talks to real TrueFoundry.
- **We do not claim full MCP transport.** `kernel_lab/server.py` exposes 4 `@mcp.tool`-decorated functions (compile / run / verify / bench) for sponsor-track registration, but at runtime the agent calls the underlying `kernel_lab.*_tool` Python APIs directly because tensor marshaling across MCP is heavy for our 30 ms inner loop. The MCP shim is *real* (it registers) but the runtime path is in-process. Documented in `kernel_lab/server.py` docstrings.
- **We do not claim a successful A/B between baseline and few-shot.** The few-shot expert library (`kernelforge/expert_examples.py`), the env-var switch (`KERNELFORGE_FEWSHOT=1`), and the A/B harness (`demo/compare_ab.py`) are all committed and working in principle. The actual live A/B never completed because the 20-op suite kept hitting LLM-generated kernels that hang `mx.eval()` indefinitely on Apple Silicon (Metal has no command-buffer timeout). The single 20-op baseline run that did finish is what the scorecard above reports.
- **We do not claim KernelForge will beat MLX's hand-tuned built-ins.** When (if) it ever produces a verified kernel, the bench harness will report the loss honestly. We did not get there in this run.
